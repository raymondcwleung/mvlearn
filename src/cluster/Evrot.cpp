...
Evrot::Evrot(Eigen::MatrixXd& X, int method)
    : mMethod(method),
      ...
      mNumData(X.rows()),
      mNumAngles(
          (int)(mNumDims * (mNumDims - 1) / 2)),  // get the number of angles
      ik(Eigen::VectorXi(mNumAngles)),
      jk(Eigen::VectorXi(mNumAngles)),
      mX(X),
      mClusters(
          std::vector<std::vector<int> >(mNumDims))  // allocate clusters vector
{
...
void Evrot::evrot() {
  ...
  if (DEBUG)
    std::cout << "Done after " << iter << " iterations, Quality is " << Q
              << std::endl;
...
double Evrot::evqual(Eigen::MatrixXd& X) {
  ...
  if (DEBUG) std::cout << "Computed quality = " << J << std::endl;
...
double Evrot::evqualitygrad(Eigen::VectorXd& theta, int angle_index) {
  ...
  for (int j = 0; j < mNumDims; j++) {    // loop over all columns
    for (int i = 0; i < mNumData; i++) {  // loop over all rows
      ...
      tmp2 = A(i, max_index_col[i]) * (Y(i, j) * Y(i, j)) /
             (max_values[i] * max_values[i] * max_values[i]);
      dJ += tmp1 - tmp2;
    }
  }
  dJ = 2 * dJ / mNumData / mNumDims;
  if (DEBUG) std::cout << "Computed gradient = " << dJ << std::endl;
