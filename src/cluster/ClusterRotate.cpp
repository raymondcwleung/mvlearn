...
std::vector<std::vector<int> > ClusterRotate::cluster(Eigen::MatrixXd& X) {
  ...
  for (int g = 2; g <= X.cols(); g++) {
    ...
    // save cluster data for max cluster or if we're near the max cluster (so
    // prefer more clusters)
    if ((e->getQuality() > mMaxQuality) ||
        (mMaxQuality - e->getQuality() <= 0.001)) {
      clusters = e->getClusters();
      vecRot = e->getRotatedEigenVectors();
    }
  }

  Eigen::MatrixXd clusterCentres =
      Eigen::MatrixXd::Zero(clusters.size(), vecRot.cols());
  ...
  for (unsigned int i = 0; i < clusters.size(); i++) {
    ...
    for (unsigned int j = 0; j < clusters[i].size(); j++) {
      double d2 =
          (vecRot.row(clusters[i][j]) - clusterCentres.row(i)).squaredNorm();
    ...
    clusters[i].clear();
    for (std::multimap<double, int>::iterator it = clusterDistance.begin();
         it != clusterDistance.end(); it++) {
